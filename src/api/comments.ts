...
export async function deleteVideoComment(commentId:string):Promise<void>{const db=requireSupabase();const{error}=await db.from('video_comments').delete().eq('id',commentId).eq('user_id',uid());if(error)throw error}
